...
const SORTS = [
  { key: 'tokens', label: 'הכי הרבה טוקנים' },
  { key: 'recent', label: 'הכי עדכני' },
];
...
export default async function (root) {
  ...
  const subtitle = sort.key === 'recent'
    ? 'הפרומפטים האחרונים שלך והתור שכל אחד מהם הפעיל. לחיצה על שורה מציגה את הפרומפט המלא.'
    : 'הפרומפטים שצרכו הכי הרבה טוקנים. לחיצה על שורה מציגה את הפרומפט המלא.';

  root.innerHTML = `
    <div class="flex" style="margin-bottom:14px">
      <h2 style="margin:0;font-size:17px;letter-spacing:-0.005em">פרומפטים</h2>
    ...
    <div class="card">
      ...
      <table id="prompts">
        <thead><tr>
          <th>${sort.key === 'recent' ? 'מתי' : 'עלות זיכרון'}</th>
          <th>פרומפט</th>
          <th>מודל</th>
          <th class="num">טוקנים</th>
          <th class="num">קריאות זיכרון</th>
          <th>שיחה</th>
        </tr></thead>
        <tbody>
          ${rows.map((r,i) => `
            ...
            </tr>`).join('') || '<tr><td colspan="6" class="muted">אין פרומפטים עדיין</td></tr>'}
        </tbody>
      </table>
    </div>
    <div id="drawer"></div>
      ...
      drawer.innerHTML = `
        <div class="card">
          ...
          <div class="flex" style="margin-top:12px;flex-wrap:wrap;gap:14px">
            ...
            <span class="muted">${fmt.int(r.billable_tokens)} טוקנים מחויבים · ${fmt.int(r.cache_read_tokens)} קריאות זיכרון · עלות זיכרון משוערת ${fmt.usd4(r.estimated_cost_usd)}</span>
            <span class="spacer"></span>
            <a href="#/sessions/${encodeURIComponent(r.session_id)}">פתח שיחה ←</a>
          </div>
        </div>`;
