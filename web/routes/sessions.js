...
async function renderList(root) {
  ...
  root.innerHTML = `
    <div class="card">
      <h2>שיחות</h2>
      <table>
        <thead><tr><th>התחלה</th><th>פרויקט</th><th class="num">סשנים</th><th class="num">טוקנים</th><th>שיחה</th></tr></thead>
        <tbody>
          ${list.map(s => `
            <tr>
            ...
            </tr>`).join('') || '<tr><td colspan="5" class="muted">אין שיחות עדיין</td></tr>'}
        </tbody>
      </table>
    </div>`;
}
...
async function renderSession(root, id) {
  ...
  const typeLabel = t => ({ user: 'משתמש', assistant: 'עוזר', system: 'מערכת' })[t] || t;

  root.innerHTML = `
    <div class="card">
      <h2 style="display:flex;align-items:center">
        <span>שיחה ${fmt.htmlSafe(id.slice(0,8))}…</span>
        <span class="spacer"></span>
        <a href="#/sessions" class="muted">→ כל השיחות</a>
      </h2>
      <div class="flex muted" style="font-family:var(--mono);font-size:12px;flex-wrap:wrap;gap:14px">
        ...
        <span>${turns.length} רשומות</span>
        <span>${fmt.int(totalIn)} קלט · ${fmt.int(totalOut)} פלט · ${fmt.int(totalCacheRd)} זיכרון</span>
      </div>
    </div>
    ...
    <div class="card" style="margin-top:16px">
      <h3>סשן-אחר-סשן</h3>
      <table>
        <thead><tr><th>שעה</th><th>סוג</th><th>מודל</th><th class="blur-sensitive">פרומפט / כלים</th><th class="num">קלט</th><th class="num">פלט</th><th class="num">זיכרון</th></tr></thead>
        <tbody>
          ${turns.map(t => {
            ...
            return `<tr>
              ...
              <td>${typeLabel(t.type)}${t.is_sidechain ? ' <span class="badge">צד</span>' : ''}</td>
