...
const ROUTES = {
  '/overview': { loader: () => import('/web/routes/overview.js'), label: 'סקירה כללית' },
  '/prompts':  { loader: () => import('/web/routes/prompts.js'),  label: 'פרומפטים' },
  '/sessions': { loader: () => import('/web/routes/sessions.js'), label: 'שיחות' },
  '/projects': { loader: () => import('/web/routes/projects.js'), label: 'פרויקטים' },
  '/skills':   { loader: () => import('/web/routes/skills.js'),   label: 'סקילים' },
  '/tips':     { loader: () => import('/web/routes/tips.js'),     label: 'טיפים לחיסכון' },
  '/settings': { loader: () => import('/web/routes/settings.js'), label: 'הגדרות' },
};
...
function buildTopbar() {
  ...
  wrap.innerHTML = `
    <a href="#/overview" class="brand" style="text-decoration:none">
      <img src="/web/assets/brand/logo-dn.png" alt="DN">
      <div class="stack">
        <span>מנהל הטוקנים</span>
        <span class="sub">ניתוח שימוש בקלוד קוד</span>
      </div>
    </a>
    <nav>
    ...
    <span class="pill gold" id="plan-pill">api</span>
    <span class="pill muted" title="הקיצור Ctrl+B מפעיל ומבטל טשטוש של תכנים רגישים">טשטוש פרטיות (Ctrl+B)</span>
  `;
...
async function render() {
  ...
  try {
    ...
    $('#app').innerHTML = `<div class="card"><h2>שגיאה</h2><pre>${fmt.htmlSafe(String(e.stack || e))}</pre></div>`;
  }
}
...
async function firstRun() {
  ...
  overlay.innerHTML = `
    <div class="modal">
      <h2>ברוך הבא, בואו נבחר את התוכנית שלך</h2>
      <p>הבחירה קובעת כיצד יוצגו עלויות בלוח. ניתן לשנות מאוחר יותר בהגדרות.</p>
      <select id="firstplan" style="width:100%">
        ${plans.map(([k,v]) => `<option value="${k}">${v.label}${v.monthly ? ` · $${v.monthly} בחודש` : ''}</option>`).join('')}
      </select>
      <div class="actions">
        <div class="spacer"></div>
        <button class="primary" id="firstsave">המשך</button>
      </div>
    </div>`;
