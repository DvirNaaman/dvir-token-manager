        ...
        for row in c.execute(sql, (since,)):
            ...
            if hit < 0.40:
                ...
                out.append({
                    "key": key,
                    "category": "cache",
                    "title": f"שיעור פגיעות בזיכרון נמוך בפרויקט {row['project_slug']}",
                    "body": f"שיעור הפגיעות בזיכרון עומד על {hit*100:.0f}% ב-7 הימים האחרונים. סשנים שמאתחלים את ההקשר תכופות בונים את הזיכרון מחדש בכל פעם. כדאי לשמור על סשנים ארוכים יותר או למעט באיפוסי הקשר.",
    ...
    with connect(db_path) as c:
        ...
        """, (since,)):
            ...
            out.append({
                ...
                "title": f"הקובץ {row['target']} נקרא {row['n']} פעמים",
                "body": f"הקובץ נפתח {row['n']} פעמים על פני {row['sessions']} סשנים ב-7 הימים האחרונים. סיכום הקובץ ב-CLAUDE.md או קריאה אחת לסשן ימנעו את הקריאות החוזרות.",
                "scope": row["target"],
            })
        for row in c.execute("""
        ...
        """, (since,)):
            ...
            out.append({
                ...
                "title": f"הפקודה `{row['target']}` רצה {row['n']} פעמים",
                "body": f"פקודת bash זו רצה {row['n']} פעמים ב-7 הימים האחרונים. כדאי לשקול דגל watch או alias בשל.",
                "scope": row["target"],
            })
    return out
...
def right_size_tips(db_path, today_iso: Optional[str] = None) -> List[dict]:
    ...
    return [{
        ...
        "title": f"{row['n']} סשנים קצרים של Opus היו יכולים לרוץ ב-Sonnet",
        "body": f"סשני Opus עם פחות מ-500 טוקני פלט עלו כ-${api_opus:.2f} ב-7 הימים האחרונים. ב-Sonnet הם היו עולים כ-${api_sonnet:.2f} (חיסכון פוטנציאלי כ-${savings:.2f}).",
        ...
        if big and (big["n"] or 0) >= 5:
            ...
            if not _is_dismissed(db_path, key):
                out.append({
                    ...
                    "title": f"{big['n']} תוצאות כלי מעל 50 אלף טוקנים השבוע",
                    "body": f"הגודל הממוצע הוא {int(big['avg_t']):,} טוקנים. כדאי לצנרר פלט ארוך של Bash ל-head/tail ולבקש קריאות קובץ ממוקדות יותר.",
                    "scope": "result-50k+",
                })
        for row in c.execute("""
        ...
        """, (since,)):
            if (row["max_t"] or 0) > 6 * (row["mean_t"] or 1) and (row["max_t"] or 0) > 50_000:
                ...
                out.append({
                    ...
                    "title": f"לסאב-אגנט {row['agent_id']} יש חריגות עלות",
                    "body": f"ההפעלה הגדולה ביותר השתמשה ב-{int(row['max_t']):,} טוקנים מול ממוצע של {int(row['mean_t']):,}. שווה לבדוק מה הן עשו אחרת.",
                    "scope": row["agent_id"],
                })
    return out
