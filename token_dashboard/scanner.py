...
def scan_dir(projects_root: Union[str, Path], db_path: Union[str, Path]) -> dict:
    ...
    with connect(db_path) as conn:
        for p in root.rglob("*.jsonl"):
            ...
            # Commit per file so other writers (settings POST, plan changes) can
            # slip in between files. Without this, a 575MB initial scan held
            # one giant transaction for minutes and blocked all UI writes.
            conn.commit()
