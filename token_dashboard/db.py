...
def init_db(path: Union[str, Path]) -> None:
    ...
    with sqlite3.connect(path, timeout=5.0) as c:
        # WAL is persistent across connections - set it once at init.
        # Lets the SSE/scanner thread and the request threads coexist.
...
def connect(path: Union[str, Path]):
    # 5s busy_timeout lets readers wait briefly when the scanner thread is writing;
    # WAL mode (set once in init_db) lets readers and writers coexist without blocking.
